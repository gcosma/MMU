...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, opt, images, tags, labels, test=None):
        ...
        if test is None:
            ...
            #opt.query_size + opt.training_size: opt.query_size * 2 + opt.training_size
            #self.query_labels = labels[0: opt.query_size]
            self.query_labels = labels[opt.query_size + opt.training_size: opt.query_size * 2 + opt.training_size]
            #np.concatenate((images[:opt.query_size + opt.training_size], images[opt.query_size * 2 + opt.training_size:]), axis=0)
            #self.db_labels = labels[opt.query_size: opt.query_size + opt.db_size]
            self.db_labels = labels[opt.query_size: opt.query_size + opt.db_size]
            if test == 'image.query':
                #self.images = images[0: opt.query_size]
                self.images = images[opt.query_size + opt.training_size: opt.query_size * 2 + opt.training_size]
            elif test == 'image.db':
                #self.images = images[opt.query_size: opt.query_size + opt.db_size]
                self.images = np.concatenate((images[:opt.query_size + opt.training_size], images[opt.query_size * 2 + opt.training_size:]), axis=0)
            elif test == 'text.query':
                #self.tags = tags[0: opt.query_size]
                self.tags = tags[opt.query_size + opt.training_size: opt.query_size * 2 + opt.training_size]
            elif test == 'text.db':
                #self.tags = tags[opt.query_size: opt.query_size + opt.db_size]
                self.tags = np.concatenate((tags[:opt.query_size + opt.training_size], tags[opt.query_size * 2 + opt.training_size:]), axis=0)

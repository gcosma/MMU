...
from utils import calc_map_k, Visualizer, calc_map_k_classes, calc_map_k_ind_extract, calc_map_k_custom_class1
...
from visualize import log_loss_values
...
def train(**kwargs):
    ...
    #images_train, images, tags, labels = load_data(opt.data_path, type=opt.dataset)
    train_data = Dataset(opt, images, tags, labels)
    #train_data = Dataset(opt, images_train, tags, labels)
    ...
    generator = GEN(opt.dropout, opt.image_dim, opt.text_dim, opt.hidden_dim, opt.bit,
    ...
    #tri_loss = TripletLoss(opt, reduction='none')
    ...
    for epoch in range(opt.max_epoch):
        ...
        for i, (ind, img, txt, label) in tqdm(enumerate(train_dataloader)):
            ...
            '''
            # Singled tri loss
            each_tri_i2t = tri_loss(h_i, labels, target=h_t, margin=opt.margin)
            each_tri_t2i = tri_loss(h_t, labels, target=h_i, margin=opt.margin)
            all_tri_i2t = torch.mean(each_tri_i2t)
            all_tri_t2i = torch.mean(each_tri_t2i)
            weighted_cos_tri = all_tri_i2t + all_tri_t2i
            '''
            ...
            '''
            # Single sample loss extraction
            singled_loss_adver_feature = -torch.log(torch.sigmoid(discriminator.dis_feature(f_t)))
            singled_loss_adver_hash = -torch.log(torch.sigmoid(discriminator.dis_hash(h_t_detach)))
            singled_i_ql = torch.pow(B_i[ind, :] - h_i, 2)
            singled_t_ql = torch.pow(B_i[ind, :] - h_t, 2)
            singled_err_values = log_loss_values(opt.alpha, each_tri_i2t.detach(), each_tri_t2i.detach(), opt.beta, singled_i_ql.detach(), singled_t_ql.detach(), opt.gamma,
                            singled_loss_adver_feature.detach(), singled_loss_adver_hash.detach())
            for e in range(0, len(singled_err_values)):
                epoch_error_values.append(singled_err_values[e])
            '''
            optimizer.zero_grad()
            err.backward()
            optimizer.step()
            ...
            e_loss = err + e_loss
        '''
        for i in range(0, len(sum_err_values)):
            sum_err_values[i] = sum_err_values[i] + epoch_error_values[i]
        '''
...
def test(**kwargs):
    ...
    class1_labels = mat_classes['LAllClassOne']
    class1_query_label = class1_labels[10000:12000]
    class1_retrieval_label = np.concatenate((class1_labels[:10000], class1_labels[12000:]), axis=0)

    stacked_class1_query_label = np.stack(class1_query_label)
    class1_query_label_tensor = torch.from_numpy(stacked_class1_query_label)

    stacked_class1_retrieval_label = np.stack(class1_retrieval_label)
    class1_query_retrieval_tensor = torch.from_numpy(stacked_class1_retrieval_label)

    mapi2t, class1_mapi2t = calc_map_k_custom_class1(qBX, rBY, query_labels, db_labels, class1_query_label_tensor, class1_query_retrieval_tensor)
    mapt2i, class1_mapt2i = calc_map_k_custom_class1(qBY, rBX, query_labels, db_labels, class1_query_label_tensor, class1_query_retrieval_tensor)
    print('...test MAP: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapi2t, mapt2i))
    print('...test MAP for Class 1 higher standard: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (class1_mapi2t, class1_mapt2i))
    '''

    '''
    class_count = []
    for i in range (0, len(db_labels[0])):
        class_count.append(0)
    for i in range (0, len(db_labels)):
        for e in range (0, len(db_labels[i])):
            if db_labels[i][e] == 1:
                class_count[e] += 1
    for i in range (0, len(class_count)):
        print("Class", i+1, ":", class_count[i])
    '''

    '''
    mapi2t = calc_map_k(qBX, rBY, query_labels, db_labels)
    mapt2i = calc_map_k_ind_extract(qBY, rBX, query_labels, db_labels)
    print('...test MAP: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapi2t, mapt2i))
    gui.run_gui()
    '''
    ...
    '''
    print("Image to text:")
    mapi2t = calc_map_k_classes(qBX, rBY, query_labels, db_labels)
    print("Text to image")
    mapt2i = calc_map_k_classes(qBY, rBX, query_labels, db_labels)
    print('...test MAP: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapi2t, mapt2i))
    '''

    '''
    
    mapk1i2t = calc_map_k(qBX, rBY, query_labels, db_labels, 1)
    mapk1t2i = calc_map_k(qBY, rBX, query_labels, db_labels, 1)
    mapk5i2t = calc_map_k(qBX, rBY, query_labels, db_labels, 5)
    mapk5t2i = calc_map_k(qBY, rBX, query_labels, db_labels, 5)
    mapk10i2t = calc_map_k(qBX, rBY, query_labels, db_labels, 10)
    mapk10t2i = calc_map_k(qBY, rBX, query_labels, db_labels, 10)
    print('...test MAP@1: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapk1i2t, mapk1t2i))
    print('...test MAP@5: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapk5i2t, mapk5t2i))
    print('...test MAP@10: MAP(i->t): %3.4f, MAP(t->i): %3.4f' % (mapk10i2t, mapk10t2i))
    '''

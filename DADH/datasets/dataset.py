...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, opt, images, tags, labels, test=None):
        ...
        '''
        if opt.flag == 'mir' or 'mirunpair':
            query_index = all_index[10000:12000]
            training_index = all_index[:10000]
            db_index = np.concatenate((all_index[:10000], all_index[12000:]), axis=0)
        else:
            query_index = all_index[opt.query_size + opt.training_size:opt.query_size * 2 + opt.training_size]
            print("Query len:", len(query_index))
            training_index = all_index[opt.query_size: opt.query_size + opt.training_size]
            print("Training len:", len(training_index))
            db_index = np.concatenate((all_index[:opt.query_size + opt.training_size], all_index[opt.query_size * 2 + opt.training_size:]), axis=0)
            print("DB len:", len(db_index))

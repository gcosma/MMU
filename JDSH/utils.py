...
def calculate_top_map_classes(qu_B, re_B, qu_L, re_L, topk):
    """
    :param qu_B: {-1,+1}^{mxq} query bits
    :param re_B: {-1,+1}^{nxq} retrieval bits
    :param qu_L: {0,1}^{mxl} query label
    ...
    """
    num_query = qu_L.shape[0]
    #label_check = qu_L.cpu()
    #label_check = label_check.tolist()
    label_check = qu_L.tolist()
    map_classes = []
    map_classes_count = []
    ...
    for iter in range(num_query):
        ...
        new_label_check = [int(o) for o in label_check[iter]]
        for e in range(0, len(label_check[iter])):
            if new_label_check[int(e)] == 1:
                map_classes[e] += np.mean(count / tindex)
    ...
    print("Overall:", float(topkmap / num_query))
    return topkmap
